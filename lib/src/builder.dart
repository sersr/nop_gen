...
String genTableDb(
  ...
  final u = columnInfos.map(
    (e) =>
        'if($tableName.${e.name} != null) update.${e.name}.set($tableName.${e.name});\n',
  );
  // update
  buffer
...
String genStatement(
  ...
  final items = columnInfos.map(
    (e) => 'T get ${e.name} => item(table.${e.name}) as T; \n',
  );
  final tableItems = columnInfos.map(
    (e) => 'T get ${lowTableName}_${e.name} => ${e.name}; \n',
  );
  buffer
    ..writeAll(items, '\n')
    ..write('\n\n')
  ...
  final joinTableItems = columnInfos.map(
    (e) =>
    ...
    for (var i in e.metadata.annotations) {
      ...
      if (nopDbItemMeta != null) {
        ...
        if (isSameType<NopDbItem>(typeName)) {
          ...
          final addPrimaryKey = nopDbItemMeta
              .getField('primaryKey')
              ?.toBoolValue();
